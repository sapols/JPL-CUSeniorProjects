...
public class MapFrame extends JFrame {
    ...
    java.util.List<Coordinate> curiosityPath = getCuriosityPathInPixels();
    ...
    public MapFrame(java.util.List<? extends Coordinate> path, String mapImagePath) {
        try {
            ...
            MapPanel imagePanel = new MapPanel(this, path, curiosityPath, backgroundImage, mapImagePath);
    ...
    /**
     * Returns an ArrayList<Coordinate> that is Curiosity's path in pixels.
     */
    public static ArrayList<Coordinate> getCuriosityPathInPixels() {
        ArrayList<Double[]> curiosityLatLon = getCuriosityPathInLatLong();
        ArrayList<Coordinate> curiosityPath = new ArrayList<Coordinate>();

        for (int i = 0; i < curiosityLatLon.size(); i++) {
            double lat = curiosityLatLon.get(i)[0];
            double lon = curiosityLatLon.get(i)[1];

            Coordinate xyCoord = convertCuriosityLatLonToCoord(lat, lon);
            curiosityPath.add(xyCoord);
        }
    ...
    /**
     * Returns an ArrayList<Double[]> that is Curiosity's path in lat/long.
     */
    public static ArrayList<Double[]> getCuriosityPathInLatLong() {
        ...
        ArrayList<Double[]> curiosityLatLon = getCuriosityPathFromImagePixels(pixels);

        return curiosityLatLon;
    }

    /**
     * Given latitude and longitude values for a point in Curiosity's path,
     * return a Coordinate holding those values as pixels.
     */
    public static Coordinate convertCuriosityLatLonToCoord(double lat, double lon) {
        double leftBound = 135;
        double bottomBound = -30;

        double diffX = lon - leftBound;
        double pixelX = diffX * 256.0;

        double diffY = lat - bottomBound;
        double pixelY = diffY * 256;

        return new Coordinate((int) pixelX, (int) pixelX);
    }
...
//    }
     ...
     */
    public static ArrayList<Double[]> getCuriosityPathFromImagePixels(Coordinate[] imagePixels) {
        ArrayList<Double[]> curiosityPath = new ArrayList<Double[]>();
        ...
        for (int i = 0; i < imagePixels.length; i++) {
            ...
            double latDouble = convertCuriosityLatLonToDouble(lat);
            double lonDouble = convertCuriosityLatLonToDouble(lon);

            Double[] latLon = { latDouble, lonDouble };
    ...
    /*
     * Convert a String representing a latitude or longitude value to a number
     */
    public static Double convertCuriosityLatLonToDouble(String latLonStr) {
        int degrees = Integer.parseInt(latLonStr.substring(0, latLonStr.indexOf('º')));
        double minutes = Double.parseDouble(latLonStr.substring(latLonStr.indexOf('º')+1, latLonStr.indexOf('\'')));
        char direction = latLonStr.charAt(latLonStr.indexOf('\'')+1);

        if (direction == 'S')
            return -(degrees + (minutes/60.0)); //latitude should be negative
        else if (direction == 'E')
            return Math.abs(degrees + (minutes/60.0)); //longitude should be positive
        else
            return degrees + (minutes/60.0); //shouldn't ever get here
    }

    /*
     * Print the contents of an ArrayList<Double[]> that is Curiosity's path in lat/lon.
     */
    public static void printCuriosityPath(ArrayList<Double[]> curiosityPath) {
